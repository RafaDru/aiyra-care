/**
 * Orquestra E2E business-full (mesma sequência do workflow ci-e2e-business-full.yml).
 *   node scripts/ci-e2e-business-full.mjs --skip-api-start   # API já em :3010
 */
...
for (const key of required) {
  if (!process.env[key]?.trim()) {
    console.error(`ci-e2e-business-full: ${key} obrigatório`)
    process.exit(1)
  }
}
...
function run(label, command, args, opts = {}) {
  ...
  if (r.status !== 0) {
    console.error(`ci-e2e-business-full FAILED: ${label}`)
...
try {
  run('playwright:business-full', 'npm', ['run', 'test:e2e:business-full'])
} finally {
  if (apiChild?.pid) {
    try {
