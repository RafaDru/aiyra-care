...
| **Hermes Pardini** | **Incremental:** `GET /pedidos` desde `max(exam_date)−14d` ou `lastSync−14d`; expande `GET /pedidos/{id}/exames` | Dedup `hermes_pardini:{pedido}:{exame}` em `notes` | PDF laudo (`POST /pedidos/{id}/download`) não importado ainda | — |
