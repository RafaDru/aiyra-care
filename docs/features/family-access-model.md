...
2. **Menu «Sua família»** (`/family`) e atalho no Início: gerenciar famílias, vincular perfis, convidar cuidadores. URL legada `/settings/family` redireciona.
...
| UI | `/family` (menu lateral), atalho no Início, dashboard agrupado, `PatientAccessGrantsDrawer` no perfil |
