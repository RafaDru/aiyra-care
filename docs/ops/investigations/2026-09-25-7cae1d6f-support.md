...
- **Re-análise automação:** 2026-09-25 (lane development_support)
...
1. **Erro de render no Dashboard ou filho direto** — `Form.useWatch` / `MaskedDatePicker`, agrupamento de pacientes por categoria, ou seções `DashboardDayToDaySection` / `DayToDayDiscoveryHub` com estado inconsistente após resposta parcial da API.
2. **Erro em shell compartilhado na rota `/`** — `AvaGlobalDock`, seletor de círculo ativo, `RuntimeDegradedBanner` ou tour no `AppLayout` (mesmo fingerprint em outros chamados na rota `/` sugere padrão recorrente).
3. **Race no primeiro paint** — `AuthContext` / `ActiveCareCircleContext` vs mount do Dashboard (menos provável sem `properties.component` no PG).
...
- `docs/ops/SUPPORT_REPORTS.md` — triagem `technical_bug`: correlacionar fingerprint + rota.
- **Correlação:** mesmo fingerprint `036967be53b57d23` em investigations `96a2e898`, `1b55aea2`, `07c77172`, `225ff8f1` (rota `/`, integration).

## Próximo passo humano

1. No host **integration** (PG local), correlacionar chamado e erros:
...
2. Inspecionar `properties->>'component'` em `client_errors` com fingerprint `036967be53b57d23` próximos ao horário do chamado.
3. Reproduzir em `http://localhost:5173/` com a mesma conta/círculo; DevTools antes do overlay do boundary.
4. Fix de produto só após componente identificado; **Tier 1** se trivial nos gates (`docs/ops/automations/TIER1_GATES.md`).

## Categoria (revisão agente)

- **Adequada:** `technical_bug` — boundary + fingerprint de `client_errors`.
- **Sugestão:** manter; escalar se volume alto do mesmo FP em 24h (runbook SUPPORT_REPORTS).
