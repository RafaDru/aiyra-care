...
import type { GoogleCalendarService } from '../../../application/calendar/google-calendar.service.js'
...
export class GoogleCalendarController {
  ...
  async oauthCallback(req: AuthenticatedRequest, reply: FastifyReply) {
    ...
    try {
      ...
      return reply.redirect(`${webBase}${returnTo}${returnTo.includes('?') ? '&' : '?'}calendar=connected`)
    } catch (err) {
