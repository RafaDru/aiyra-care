...
function alertWithDetectedAt(
  snapshot: OpsMetricsSnapshot,
  partial: Omit<OpsAlert, 'detectedAt'> & { detectedAt?: string },
): OpsAlert {
  return {
    ...partial,
    detectedAt: partial.detectedAt ?? snapshot.generatedAt,
  }
}

export function evaluateOpsAlerts(snapshot: OpsMetricsSnapshot): OpsAlert[] {
  const alerts: OpsAlert[] = []
  const defaultDetectedAt = snapshot.generatedAt

  const probe = snapshot.probe
  if (probe) {
    const probeDetectedAt = probe.checkedAt
    if (!probe.api.ok) {
      alerts.push(alertWithDetectedAt(snapshot, {
        id: 'infra_api_down',
        severity: 'critical',
        category: 'infra',
        message: `API health falhou (${probe.api.status ?? 'no response'})`,
        detectedAt: probeDetectedAt,
  ...
  if (workerStaleMinutes != null && workerStaleMinutes >= WORKER_STALE_MINUTES) {
    alerts.push(alertWithDetectedAt(snapshot, {
      ...
      detectedAt: snapshot.ops?.workerLastTickAt ?? defaultDetectedAt,
      details: {
