...
import { appendReferralToShareUrl, isValidReferralCode } from '../../domain/referral/referral-code.js'
...
export class ClinicalExportShareService {
  ...
  }): Promise<{ token: string; expiresAt: string; shareUrl: string; referralCode: string | null }> {
    const token = randomBytes(32).toString('base64url')
    const tokenHash = hashToken(token)
    const expiresAt = new Date(Date.now() + (args.ttlMs ?? DEFAULT_TTL_MS))
    const referralCode =
      args.referralCode && isValidReferralCode(args.referralCode) ? args.referralCode : null

    await this.pool.query(
      `INSERT INTO clinical_export_shares (patient_id, token_hash, mode, expires_at, created_by, referral_code, recipient_email)
      ...
      [
        ...
        args.createdBy ?? null,
        referralCode,
        args.recipientEmail?.trim().toLowerCase() ?? null,
      ],
    )

    const base = process.env.PUBLIC_WEB_URL ?? 'http://localhost:5173'
    const bareUrl = `${base.replace(/\/$/, '')}/clinical-export/${token}`
    const shareUrl = appendReferralToShareUrl(bareUrl, referralCode)
    return {
      token,
  ...
  async markOpened(token: string, referralFromQuery?: string | null): Promise<void> {
    const tokenHash = hashToken(token)
    const ref =
      referralFromQuery && isValidReferralCode(referralFromQuery) ? referralFromQuery : null
    await this.pool.query(
      `UPDATE clinical_export_shares
